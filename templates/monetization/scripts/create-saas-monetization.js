...
const TEMPLATES_REPO = process.env.SAAS_TEMPLATES_PATH || path.join(process.env.HOME || os.homedir(), 'Projects/saas-monetization-templates/templates')
...
class SaaSMonetizationBootstrap {
  ...
  async run() {
    ...
    if (!fs.existsSync(this.templatePath)) {
      ...
      console.error('Please ensure saas-monetization-templates is cloned to ~/Projects/saas-monetization-templates/')
      process.exit(1)
    }
  ...
  async collectConfiguration() {
    ...
    try {
      ...
      this.config.projectName = await question('Project name: ') || path.basename(this.projectRoot)
      this.config.description = await question('Project description: ') || `${this.config.projectName} - Professional SaaS platform`
      this.config.domain = await question('Domain (e.g. yoursite.com): ') || 'yoursite.com'
      this.config.companyName = await question('Company/Author name: ') || 'Your Company'
      this.config.supportEmail = await question('Support email: ') || 'support@yoursite.com'
      this.config.governingLaw = await question('Governing law (default Delaware, USA): ') || 'Delaware, USA'
      this.config.venue = await question('Venue (default State and Federal courts in Delaware): ') || 'State and Federal courts in Delaware'

      console.log('\\n💰 Pricing Configuration')
      console.log('─────────────────────────')
      this.config.proPrice = await question('Pro tier monthly price (default $39): ') || '39'
      this.config.enterprisePrice = await question('Enterprise tier monthly price (default $197): ') || '197'
      this.config.founderDiscount = await question('Founder discount % (default 50): ') || '50'

      console.log('\\n🎯 Feature Configuration')
      console.log('─────────────────────────')
      this.config.premiumFeatures = await question('Premium features (comma-separated): ') || 'Advanced Analytics,Priority Support,Custom Integrations'
      this.config.freeTierFeatures = await question('Free tier features (comma-separated): ') || 'Basic Features,Community Support'
  ...
  generateDerivedConfig() {
    // Calculate pricing
    this.config.founderProPrice = (parseFloat(this.config.proPrice) * (1 - parseFloat(this.config.founderDiscount) / 100)).toFixed(2)
    this.config.founderEnterprisePrice = (parseFloat(this.config.enterprisePrice) * (1 - parseFloat(this.config.founderDiscount) / 100)).toFixed(2)

    // Generate project slug (for directory names, license prefixes)
    this.config.projectSlug = this.config.projectName.toLowerCase().replace(/[^a-z0-9]/g, '-')

    // Generate license prefix (uppercase, truncated)
    this.config.licensePrefix = this.config.projectName.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 3) || 'SAS'

    // Parse features
    this.config.proFeatureList = this.config.premiumFeatures.split(',').map(f => f.trim())
    this.config.freeFeatureList = this.config.freeTierFeatures.split(',').map(f => f.trim())
    ...
    this.config.venue = this.config.venue || 'State and Federal courts in Delaware'
  }
  ...
  async processTemplate(sourcePath, targetPath) {
    ...
    content = content
      ...
      .replace(/{{ENTERPRISE_FOUNDER_PRICE}}/g, this.config.founderEnterprisePrice)
      ...
      .replace(/{{PRO_FEATURE_1}}/g, this.config.proFeatureList[0] || 'Advanced Features')
      .replace(/{{PRO_FEATURE_2}}/g, this.config.proFeatureList[1] || 'Priority Support')
      .replace(/{{PRO_FEATURE_3}}/g, this.config.proFeatureList[2] || 'Custom Integrations')
  ...
  async updatePackageJson() {
    ...
    console.log('✅ Updated package.json with monetization scripts and dependencies')
  }
...
`
    ...
    fs.writeFileSync(path.join(this.projectRoot, 'MONETIZATION_GUIDE.md'), guide)
