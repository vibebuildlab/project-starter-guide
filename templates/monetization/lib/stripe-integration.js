...
class StripeIntegration {
  constructor() {
    ...
    this.priceIds = {
      PRO_MONTHLY: process.env.STRIPE_PRICE_ID_PRO || 'price_pro_monthly_placeholder',
      ENTERPRISE_MONTHLY: process.env.STRIPE_PRICE_ID_ENTERPRISE || 'price_enterprise_monthly_placeholder',
      PRO_FOUNDER: process.env.STRIPE_PRICE_ID_PRO_FOUNDER || 'price_pro_founder_placeholder',
      ENTERPRISE_FOUNDER: process.env.STRIPE_PRICE_ID_ENTERPRISE_FOUNDER || 'price_enterprise_founder_placeholder'
    }
  ...
  getSigningSecret() {
    ...
    if (process.env.NODE_ENV === 'production' && secret.includes('{{PROJECT_SLUG}}-dev-secret-change-in-prod')) {
      throw new Error('LICENSE_SIGNING_SECRET is required in production for secure license signing.')
    }
    return secret
  }
   ...
   */
  requireStripe(message = 'Stripe is not configured. Set STRIPE_SECRET_KEY to enable billing flows.') {
    if (!this.stripe) {
  ...
  async createCheckoutSession({
    ...
    if (!initialized) {
      throw new Error('Stripe is not configured. Set STRIPE_SECRET_KEY to create checkout sessions.')
    }
    this.requireStripe()
    ...
    try {
      ...
      if (fs.existsSync(licenseFile)) {
        ...
        if (license.licenseKey === licenseKey) {
          // Validate signature
          const isValid = this.verifyLicenseSignature(license.payload, license.signature)
      ...
      const customerMatch = licenseKey.match(/{{LICENSE_PREFIX}}-([A-F0-9]{4})-([A-F0-9]{4})-([A-F0-9]{4})-([A-F0-9]{4})/)
      ...
      if (process.env.NODE_ENV === 'production') {
        return { valid: false, error: 'Stripe validation unavailable in production. Check STRIPE_SECRET_KEY.' }
      }

      console.log('⚠️ Using development license validation (Stripe lookup not implemented)')
  ...
  async storeLicenseLocally(licenseData) {
    try {
      ...
      console.log(`🎁 Founder: ${licenseData.payload.isFounder ? 'Yes' : 'No'}`)
